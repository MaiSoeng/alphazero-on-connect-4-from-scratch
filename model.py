...
def valid_moves(board):
    ...
    columns = list(range(board.shape[1]))

    for column in range(board.shape[1]):
        if column_full(board, column) == True:
...
def four_in_a_row_diagonal_down_right(board):
    ...
    for row in range(board.shape[0] - 3):
        for column in range(board.shape[1]- 3):
            window = board[row:row + 4, column:column + 4].diagonal()
...
def step_env(board, column, player):
    ...
    nxt = other_player(player)
    return (new_board, done, winner, nxt)
    pass
...
def encode_board(board, current_player):
    ...
    other_plane = (board == other).astype(np.float32)
    
    encode = np.stack([current_plane, other_plane], axis = 0)
    return encode
    pass
...
def init_conv_backbone(in_channels=2, hidden_channels=16):
    ...
    return torch.nn.Sequential(
        torch.nn.Conv2d(in_channels, hidden_channels, kernel_size = 3,
        stride = 1, padding = 1),
        torch.nn.ReLU(),
        torch.nn.Conv2d(hidden_channels, hidden_channels, kernel_size = 3,
        stride = 1, padding = 1),
        torch.nn.ReLU()
    )
    pass
